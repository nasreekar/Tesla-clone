...
describe("<CustomButton />", () => {
  ...
  it("renders correctly", () => {
    const rendered = render(<CustomButton type = {buttonEnum.Primary} title={"order now"} onPress ={()=>({})} />);
    expect(rendered).toMatchSnapshot();
  });

  it("renders primary button correctly",() => {
    const rendered = render(<CustomButton type = {buttonEnum.Primary} title={"order now"} onPress ={()=>({})} />);
    const textComponent = rendered.getByTestId('customButtonTitle');
    expect(textComponent.props.children).toEqual('order now');
  })

  it("renders secondary button correctly",() => {
    const rendered = render(<CustomButton type = {buttonEnum.Secondary} title={"learn more"} onPress ={()=>({})} />);
    const textComponent = rendered.getByTestId('customButtonTitle');
    expect(textComponent.props.children).toEqual('learn more');
  })

  it('primary button should be rendered in black color', () => {
    const wrapper = render(<CustomButton type = {buttonEnum.Primary} title={"order now"} onPress ={()=>({})} />);
    const childStyles = wrapper.toJSON().children[0].props.style;
  ...
  it('secondary button should be rendered in black color', () => {
